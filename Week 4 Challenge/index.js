...
//Store the results in the following variables. Largest profit and loss will need the date and amount stored, as per the output format specs.
//We will need total change to work out average, even though this doesn't form part of the output.
let totalLength = 0;
let totalProfits = 0;
let totalChange = 0;
let averageChange = 0;
...
for (var rowIndex = 0; rowIndex < finances.length; rowIndex++) {
    ...
    //start with 0.
    //check last increase. if bigger than last increase -1, track it within a loop. 
    if (rowIndex > 0) {
        let change = finances[rowIndex][1] - finances[rowIndex - 1][1];
        totalChange += change; //total change tracked here for average change as mentioned.
